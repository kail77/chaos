// OEMUI.cpp : Main file for OEM UI test module.
...
LONG APIENTRY OEMDocUICallBack_ImgFormat(PCPSUICBPARAM pCallbackParam, POEMCUIPPARAM pOEMUIParam);
...
INT_PTR CALLBACK ImagePageProc(HWND hDlg, UINT uiMsg, WPARAM wParam, LPARAM lParam);
...
////////////////////////////////////////////////////////////////////////////////
//
// Add OptItems to device or document-Advance property
//
...
////////////////////////////////////////////////////////////////////////////////
//
// Add OptItems to Docprop-Advance
//
static HRESULT hrDocumentPropertyPage(DWORD dwMode, POEMCUIPPARAM pOEMUIParam)
{
    ...
    else
    {
        ...
        pOEMUIParam->OEMCUIPCallback = OEMDocUICallBack_ImgFormat;
        ...
        pOEMUIParam->pOEMOptItems[0].Sel = pOEMDev->iImageFormat;
        ...
        pOEMUIParam->pOEMOptItems[0].pOptType->pOptParam[0].pData = GetStringResource(pOEMUIParam->hOEMHeap, pOEMUIParam->hModule, IDS_BMP);
        pOEMUIParam->pOEMOptItems[0].pOptType->pOptParam[1].pData = GetStringResource(pOEMUIParam->hOEMHeap, pOEMUIParam->hModule, IDS_TIF);
    }
        ...
        case PROPSHEETUI_REASON_INIT:
            {
                ...
                POEMDEV pOEMDev = (POEMDEV) pOEMUIParam->pOEMDM;
                PROPSHEETPAGE Page;
//OutputDebugString(pOEMDev->szOutputDir);
                memset(&Page, 0, sizeof(PROPSHEETPAGE));
                Page.dwSize = sizeof(PROPSHEETPAGE);
                Page.dwFlags = PSP_DEFAULT;
                Page.hInstance = ghInstance;
                Page.pszTemplate = MAKEINTRESOURCE(IDD_DOC_PROPPAGE);
                Page.pfnDlgProc = ImagePageProc;
                Page.lParam = (LPARAM)pOEMDev; // pass to ImagePageProc
...
INT_PTR CALLBACK ImagePageProc(HWND hDlg, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uiMsg)
    {
        case WM_INITDIALOG:
			...
			pOEMDev = (POEMDEV)pPage->lParam;
        	SetDlgItemText(hDlg, IDC_SAVEPATH, pOEMDev->szOutputDir);
        	SetWindowLongPtr(hDlg, GWLP_USERDATA, (LONG_PTR)pOEMDev);
        	break;
        case WM_COMMAND:
			if(BN_CLICKED==HIWORD(wParam) && IDC_SELPATH==LOWORD(wParam))
			{
				BROWSEINFO bi;
				pOEMDev = (POEMDEV)GetWindowLongPtr(hDlg, GWLP_USERDATA);
				bi.hwndOwner = hDlg;
				bi.pidlRoot = NULL;
				bi.pszDisplayName = pOEMDev->szOutputDir;
				bi.lpszTitle = L"Output Folder:";
				bi.ulFlags = 0;
				bi.lpfn = NULL;
				bi.lParam = 0;
				bi.iImage = 0;
				...
				if(lp && pOEMDev)
				{
					SHGetPathFromIDList(lp, pOEMDev->szOutputDir);
					SetDlgItemText(hDlg, IDC_SAVEPATH, pOEMDev->szOutputDir);
				}
			}
            return TRUE;
...
LONG APIENTRY OEMDocUICallBack_ImgFormat(PCPSUICBPARAM pCallbackParam, POEMCUIPPARAM pOEMUIParam)
{
    ...
    VERBOSE("OEMDocUICallBack_ImgFormat() entry\r\n");

    switch(pCallbackParam->Reason)
    {
        case CPSUICB_REASON_APPLYNOW:
            // Update UI item data.
            pOEMDev->iImageFormat = pOEMUIParam->pOEMOptItems[0].Sel;
            break;

        case CPSUICB_REASON_KILLACTIVE:
            // Update devmode if settings have changed.
            if(pOEMDev->iImageFormat != pOEMUIParam->pOEMOptItems[0].Sel)
            {
        ...
        case CPSUICB_REASON_SETACTIVE:
            if(pOEMUIParam->pOEMOptItems[0].Sel != pOEMDev->iImageFormat)
            {
                pOEMUIParam->pOEMOptItems[0].Sel = pOEMDev->iImageFormat;
