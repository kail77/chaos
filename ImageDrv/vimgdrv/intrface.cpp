// Intrface.cpp : Interface for User Mode COM Customization DLL.
...

{
    ...
    //
    POEMPDEV pOemPDEV = (POEMPDEV)pdevobj->pdevOEM;
	if(pOemPDEV->prgbq && pOemPDEV->cPalColors>2)
	{
...

{
    ...
    *pDevOem = pOemPDEV;
	...
	pOemPDEV->iImageFormat = pOEMDM->iImageFormat;
	wcscpy_s(pOemPDEV->szOutputDir, MAX_PATH, pOEMDM->szOutputDir);
	size_t n = wcslen(pOemPDEV->szOutputDir);
	if(pOemPDEV->szOutputDir[n-1] != '\\')
	{
		pOemPDEV->szOutputDir[n] = '\\';
		pOemPDEV->szOutputDir[n+1] = 0;
	}
...
/*    *ppbResult = NULL;
    ...
    //
    pOemPDEV->nPageWidth = pBitmapInfoHeader->biWidth;
    pOemPDEV->nPageHeight += pBitmapInfoHeader->biHeight;
  pOemPDEV->bmInfoHeader.biHeight += pBitmapInfoHeader->biHeight;
  pOemPDEV->bmInfoHeader.biSizeImage += pBitmapInfoHeader->biSizeImage; // biSizeImage maybe error sometimes
    int nSize = (pBitmapInfoHeader->biWidth+31) * pBitmapInfoHeader->biBitCount/32 *4;
    nSize *= pBitmapInfoHeader->biHeight;

    // Increase the buffer by the size of the current band
    //
    if (!bGrowBuffer(pOemPDEV, nSize))
        return E_FAIL;

    if (!pIPParams->bBlankBand)     // Non-blank band
    {
        CopyMemory((pOemPDEV->pBufStart + dwBufOffset), pSrcBitmap, nSize);
    }
    else        // For blanks bands, buffer blank scanlines
    {
        memset((pOemPDEV->pBufStart + dwBufOffset), 0xff, nSize);
    }
