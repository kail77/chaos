...
public:
	...
	//ECB: if the same block is encrypted twice with the same key, the resulting ciphertext blocks are the same.
	//CBC: a ciphertext block is obtained by first xoring the plaintext block with the previous ciphertext block, and encrypting the resulting value.
	//CFB: a ciphertext block is obtained by encrypting the previous ciphertext block and xoring the resulting value with the plaintext.
...
// std::string encryt(string Data,const char * pwd);
// std::string decryt(string content,const char * pwd);
